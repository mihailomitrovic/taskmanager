...
class Zadatak {
    ...
    public static function dodaj($naziv, $korisnik, $kategorija, $opis, $slika, mysqli $konekcija) {
        ...
        if ($slika != "") {
            ...
            $dodaj->bind_param("siissi", $naziv, $korisnik, $kategorija, $opis, $datumKreiranja, $izvrsen, $slika);
        } else {
            $dodaj = $konekcija->prepare("INSERT INTO zadatak(naziv, korisnik, kategorija, opis, datumKreiranja, izvrsen) VALUES (?,?,?,?,?,?,?)");
            $dodaj->bind_param("siissis", $naziv, $korisnik, $kategorija, $opis, $datumKreiranja, $izvrsen);
        }
