...
<body>
    ...
    <div class="container_wrapper">
        ...
        <form method="post" action="" enctype="multipart/form-data" class="container_unos" id="forma">
            ...
            <h4>Slika</h4>
            <div id="slikadiv">
                <input type="file" id="real_file" accept=".jpg, .jpeg, .png" value="" hidden="hidden">
                <button type="button" id="custom_button" class="inputslika">Izaberi sliku</button>
                <label id="custom_text">NISTA</label>
            </div>
...
<script>  
    ...
    function dodajSliku() {
        ...
        $("#real_file").change(function() {
           if ($("#real_file").val() != null) {
            let vrednost = $("#real_file").val().replace("C:\\fakepath\\", "");
            $("#custom_text").text(vrednost);
           } else {
            $("#custom_text").text("GRESKA");
           }
        })
    }
    dodajSliku();
