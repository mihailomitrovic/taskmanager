...
if (isset($_POST["button"])) {
    ...
    $fileName = trim($_FILES["slika"]["name"]);
    $fileSize = trim($_FILES["slika"]["size"]);
    $tmpName = trim($_FILES["slika"]["tmp_name"]);
    ...
    if (!in_array($imageExtension, $validImageExtension)) {
        echo '<script type="text/javascript"> alert("Invalid Image Extension"); </script>'; 
        ...
        move_uploaded_file($tmpName, "../slike/", $newImageName);
    }


    if (Zadatak::dodaj($naziv, $korisnik, $kategorija, $opis, $newImageName, $konekcija)) {
