...
foreach ($zadaci as $zad) {
    ?>
    <tr name="red" id="red" >
        <td data-value="<?=$zad->zadatakID?>" onclick="vratiDetalje($(this).data('value')); proveriTekst($(this).data('value'));">
            <h5><?= $zad->naziv?></h5>
            <?php
...
<script>
    ...
    function proveriTekst(red) {
        let slika = red->$slika;
        if (slika == null) {
            $("#opisdetalji").css("background-color", "red").change();
        }
    }
    proveriTekst();
</script>
